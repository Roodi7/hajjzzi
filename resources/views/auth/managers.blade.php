@extends('layouts.app', [
    'navlinks' => [['title' => 'الرئيسية', 'link' => route('home')], ['title' => 'مدراء المساكن ', 'link' => route('user.index')]],
])
@section('title', 'إدارة مدراء المساكن')
@section('content')
                ...
                <div class="card-body">
                    ...
                    <h4 class="card-title d-flex justify-content-between mb-4">
                        ...
                        <div class="col-md-3 text-end" style="float:left">
                            <a class="btn btn-primary waves-effect waves-light" href="{{ route('user.managers') }}"><i
                                    class="bx bx-sync font-size-20 align-middle"></i></a>
                            <a href="{{ route('accommodation_manager.create') }}"
                            ...
                            <tbody>
                                @forelse ($users as $user)
                                    <tr>
                                        ...
                                        <td class="text-center align-middle">{{ $user->managedAccommodations->name }}</td>

                                        <td class="d-flex justify-content-center gap-1">
                                            <form action="{{ route('accommodation_manager.destroy', $user->id) }}" method="post">
                                                @csrf
                                                @method('DELETE')
                                                <button onclick="return confirm('هل تريد حذف المستخدم؟')" type="submit"
